...
if (isset($_POST['submit'])) {
	...
	if ($item->save()) {
		...
		log_action('item', 'New Item Added', "{$item->id}: {$item->title} at {$item->cost} form {$item->city}.");
...
if(isMobile()){
	include $dir_public_mobile.'addstuffs.php';
}elseif(isFacebook()){
	...
	if (!empty($_GET['type'])) {
		switch ($_GET['type']) {
			case 'retailer':
				include $dir_addstuffs.'retailer.php';
				break;

			case 'individual':
				include $dir_addstuffs.'individual.php';
				break;

			case 'customs':
				include $dir_addstuffs.'custom.php';
				break;
