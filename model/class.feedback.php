...
class Feedback extends DatabaseObject
{
	...
	protected static $db_fields = array('id', 'email', 'message', 'city', 'validity', 'created');
	
	public static function feedback_solved($id) {
		global $database;
	...
	public function send_notification() {
		$to_name = $this->email;
		$to = $this->email;
		$subject = "Your Feedback on Swiftdeal";
		$from = "Swiftdeal.in <info@swiftdeal.in>";
		$message 	=<<<EMAILBODY
Hi, {$this->name}
		
Thanks a lot for providing us a feedback. we are very grateful for your help.
...
EMAILBODY;
	...
	public static function find_all() {
		return self::find_by_sql("SELECT id FROM ".self::$table_name);
	}
	
	public static function find_unsolved() {
		return self::find_by_sql("SELECT id FROM ".self::$table_name." WHERE validity=1");
	}
