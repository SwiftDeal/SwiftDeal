			...
			<div class="col-md-9">
				...
				<div class="bs-example">
					<ul class="nav nav-tabs" style="margin-bottom: 15px;">
						...
						<li><a href="#withphoto" data-toggle="tab">With Photo</a></li>
					</ul>
					<div id="myTabContent" class="tab-content">
						...
						if(strlen($keywords)>2){
							echo '<div class="alert alert-success alert-dismissable">
										...
										Your search for <strong><b>'.censor($keywords).'</b></strong> returned <strong>'.$results_num.' results </strong> in '.$end.' seconds at '.$_COOKIE['dealcity'].'(<a onclick="changecity();" href="search_results.php?keywords='.$keywords.'"><u>change city</u></a>)
								   </div>';
							if ($search->total_results != 0) {
							log_action('search', "Found {$results_num} items: ", "{$keywords}");
						?>
						...
						<div class="tab-pane fade" id="withphoto">
						<?php
							foreach ($results as $result){
								$item = Item::find_by_id('id', $result->id);
								$photo = Photograph::find_by_id('item_id', $item->id);
				...
				<?php
					...
					echo '<div class="alert alert-success alert-dismissable">
							  <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
							  Get Notified when someone adds '.$keywords.' or We will Contact you when we have this.Fill the Form below.
						  </div>';
				...
				if(!empty($keywords)) {
					if ($pagination->total_pages() > 1) {
						if ($pagination->has_previous_page()) {
							echo "<a href=\"search_results.php?keywords={$keywords}&page=";
